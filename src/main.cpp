...
//  unsigned int bridgeTime4 = 3; //Defining the time of crossing constraints after three divisions
//  unsigned int bridgeTime8 = 7; //Defining the time of crossing constraints after seven divisions
  ...
  Graph catalog; //The graph which contains the catalog of forms with which forms have to be assessed in given timestep
...
//add in catalog all forms to compare with at the first crossing conra/Users/Musky//Dev/Viab-Celli
...
//
//  for(unsigned int i = formCatalog4.size(); i < boost::num_vertices(catalog); i++ )
...
//Output the config of the simulation
  ...
  char directions[4] = {'u','d','r','l'};  //The controls which indicates th direction of mitosis
  ...
  verticesPerNbCell[formContainer.count()].push_back(boost::num_vertices(g)-1);
      ...
      while( nbVertices < verticesPerTimestep[timestep-1] )
      {
          ...
          while((motherPosition < maxSize) && (formSize < nbCells))
          {
              ...
              if(form[motherPosition])
              {
                  ...
                  for(int d=0; d<4; d++ )
                  {
                      //Try to trigger a mitosis of the current cell with the current control within the current form
                      boost::dynamic_bitset<> mitoForm = form;
                      bool mitose = env->mitose(mitoForm,motherPosition,directions[d]);

                      //Calculating the subset of reachable sets or enforcing the some crossing constraints
                      //At the time of crossing constraints, compare in an automated way (rather than by the user) the reached forms to the forms of the catalog defined at the beginning
...
//                      {
//                          unsigned int inCatalog = env->existInGraph(catalog,mitoForm,formCatalog4);
//                          if(!inCatalog)
//                          mitose = false;
//                      }
//
...
//                      {
//                          unsigned int inCatalog = env->existInGraph(catalog,mitoForm,formCatalog8);
//                          if(!inCatalog)
//                          mitose = false;
//                      }
                      ...
                      if(mitose)
                      {
                          ...
                          //If there are already forms of same number of cells as the newly generated form,
                          //test if there is any redundance, also with geometrical transformation
                          if (verticesPerNbCell[newNbCells].size() != 0)
                          vertex = env->existInGraph(g, mitoForm, verticesPerNbCell[newNbCells]);
                          else
                          vertex = 0;
                          ...
                          if(vertex == 0)
                          {
                              ...
                              //In the map, record the reference of the vertex among those of the same number of cell
                              verticesPerNbCell[newNbCells].push_back(boost::num_vertices(g)-1);
                              ...
                              //Increment the number of added vertices in the current timestep
                          ...
                          {
                              //Find among the added vertices which one represent the newly created form
